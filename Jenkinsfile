pipeline {
    ...
    environment {
        ...
        BUILD_TIMESTAMP = "${new Date().format('yyyyMMdd_HHmmss')}" // 全局时间戳变量
    }
    ...
    stages {
                ...
                script {
                    sshagent([SSH_CREDENTIALS]) {
                        ...
                        ssh -o StrictHostKeyChecking=no root@${ECS_IP} '
                            ...
                            docker run --name \${CONTAINER_NAME} -v /usr/automation_pipeline/pytest_UI_automation:/pytest_UI_automation ${DOCKER_IMAGE} pytest -v -s --alluredir=/pytest_UI_automation/report/allure-results-${BUILD_TIMESTAMP} test_suites/
                        '
                        """
                    }
                }
            ...
            steps {
                script {
                    sshagent([SSH_CREDENTIALS]) {
                        ...
                        scp -o StrictHostKeyChecking=no -r root@${ECS_IP}:/usr/automation_pipeline/pytest_UI_automation/report/allure-results-${BUILD_TIMESTAMP} ${WORKSPACE}/report/
                        """
                    }
                }
            }
        ...
        stage('Generate Allure Report') {
            steps {
                script {
                    // 在 Jenkins 容器里生成 Allure 报告
                    sh 'allure generate report/allure-results-${BUILD_TIMESTAMP} --clean -o report/allure-reports-${BUILD_TIMESTAMP}'
                }
                allure([
                    reportBuildPolicy: 'ALWAYS',
                    includeProperties: false,
                    results: [[path: "report/allure-results-${BUILD_TIMESTAMP}"]]
                ])
            }
        }
    ...
    post {
        success {
            ...
            script {
                ...
                sh 'ls -R report/allure-reports-${BUILD_TIMESTAMP}/'

                // 压缩 allure-reports 文件夹
                sh "zip -r report/allure-reports-${BUILD_TIMESTAMP}.zip report/allure-reports-${BUILD_TIMESTAMP}/*"
                emailext(
                    ...
                    attachmentsPattern: "report/allure-reports-${BUILD_TIMESTAMP}.zip"  // 指定压缩文件作为附件
                )
            }
            // 清理工作区
            cleanWs() // 清理工作区
