                ...
                script {
                    sshagent([SSH_CREDENTIALS]) {
                        ...
                        ssh -o StrictHostKeyChecking=no root@${ECS_IP} '
                            ...
                            docker run -v /usr/automation_pipeline/pytest_UI_automation:/pytest_UI_automation ${DOCKER_IMAGE} pytest -v -s --alluredir=/pytest_UI_automation/report/allure-results test_suites/
                        '
                        """
                    }
                }
